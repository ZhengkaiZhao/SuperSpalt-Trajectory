...
class CameraAnimTrack implements AnimTrack {
    ...
    getPose(frame: number): Pose | null {
        const pose = this.poses.find(p => p.frame === frame);
        return pose ? {
            name: pose.name,
            frame: pose.frame,
    ...
    setPosePosition(frame: number, position: Vec3, target: Vec3, transient = false): boolean {
        const pose = this.poses.find(p => p.frame === frame);
        if (!pose || !Number.isFinite(position.x) || !Number.isFinite(position.y) ||
            !Number.isFinite(position.z) || !Number.isFinite(target.x) ||
        ...
        this.rebuildSpline();
        this.events.fire(transient ? 'track.poseUpdated' : 'track.keyUpdated', frame);
        return true;
    }
    ...
    sample(frame: number): Pose | null {
        const orderedPoses = this.activePoses();
        if (orderedPoses.length === 0) {
            return null;
        }
        if (orderedPoses.length === 1) {
            const pose = orderedPoses[0];
            return {
                name: pose.name,
                frame,
                position: pose.position.clone(),
        ...
        const spline = this.buildSpline(orderedPoses);
        const result: number[] = [];
        spline.evaluate(frame, result);
        return {
            name: `sample_${frame}`,
            frame,
            position: new Vec3(result[0], result[1], result[2]),
            target: new Vec3(result[3], result[4], result[5]),
            fov: result[6]
        };
    }
    ...
    private buildSpline(orderedPoses: Pose[]): CubicSpline {
        const duration = this.events.invoke('timeline.frames');
        ...
        for (const pose of orderedPoses) {
            points.push(pose.position.x, pose.position.y, pose.position.z);
            points.push(pose.target.x, pose.target.y, pose.target.z);
            points.push(pose.fov);
        }
        return loop ?
...
const registerCameraPosesEvents = (events: Events) => {
    ...
    events.function('camera.pose', (frame: number) => track.getPose(frame));
    events.function('camera.samplePose', (frame: number) => track.sample(frame));
    ...
    events.function('docSerialize.poseSets', (): any[] => {
        ...
        const recordedTrajectories = (events.invoke('recordedView.trajectories') as {
            id: string,
            label: string,
            targetCount: number,
            finished: boolean,
        ...
        recordedTrajectories.filter(trajectory => trajectory.poses.length > 0).forEach((trajectory) => {
            sets.push({
                name: `recorded_views_${trajectory.label}`,
                ...
                poses: trajectory.poses.map(pose => ({
                    name: pose.name,
                    frame: pose.frame,
                    position: pack3(pose.position),
                    target: pack3(pose.target),
    ...
    events.function('docDeserialize.poseSets', (poseSets: any[], documentCameraFov?: number) => {
        ...
        const isRecordedSet = (set: any) => set.role === 'recorded-view-keyframes' ||
            set.role === 'recorded-view-trajectory';
        const trackSet = poseSets.find(set => set.role === 'camera-track') ??
            poseSets.find(set => !isRecordedSet(set));
        const loadedPoses: Pose[] = (trackSet?.poses ?? []).map((docPose: any, index: number) => {
            return {
                name: docPose.name,
        ...
        const recordedSets = poseSets.filter(isRecordedSet);
        if (recordedSets.length > 0) events.invoke('recordedView.restoreAll', recordedSets);
    });
};
