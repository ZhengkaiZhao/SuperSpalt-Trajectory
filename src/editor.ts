...
const registerEditorEvents = (events: Events, editHistory: EditHistory, scene: Scene) => {
    ...
    events.on('scene.elementAdded', (element: Element) => {
        if (!autoFocusPending || element.type !== ElementType.splat) return;
        const splat = element as Splat;
        const focalPoint = splat.focalPoint();
        const radius = splat.framingRadius(focalPoint);
        if (!Number.isFinite(radius) || radius <= 1e-6 || !Number.isFinite(focalPoint.x) ||
            !Number.isFinite(focalPoint.y) || !Number.isFinite(focalPoint.z)) {
            return;
        }
        autoFocusPending = false;
        scene.camera.focus({ focalPoint, radius, speed: 0 });
    });
    ...
    events.on('camera.reset', () => {
        ...
        const splat = splats.find(item => item.visible && item.numSplats > 0);
        if (splat) {
            scene.camera.setAzimElev(initialAzim, initialElev, 0);
            const focalPoint = splat.focalPoint();
            scene.camera.focus({
                focalPoint,
                radius: splat.framingRadius(focalPoint),
                speed: 0
            });
            return;
        }
        ...
        for (const splat of selectedSplats()) {
            ...
            if (mode === 'centers') {
                ...
                for (let i = 0; i < numSplats; i++) {
                    vec4.set(centers[i * 3], centers[i * 3 + 1], centers[i * 3 + 2], 1.0);
    ...
    const setCameraMode = (mode: string) => {
        if (scene.graphicsDevice.isWebGPU) mode = 'centers';
        if (mode !== activeMode) {
            activeMode = mode;
    ...
    events.function('camera.getPose', () => {
        ...
        const focalOffset = focalPoint.clone().sub(position);
        const targetDistance = Math.max(focalOffset.length(), 1e-6);
        const target = position.clone().add(forward.mulScalar(targetDistance));
        return {
    ...
    events.on('camera.setPose', (pose: { position: Vec3, target: Vec3, fov?: number }, speed = 1) => {
        ...
        if (speed === 0) scene.camera.onUpdate(0);
    });
