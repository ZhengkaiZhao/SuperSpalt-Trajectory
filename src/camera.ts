...
class Camera extends Element {
    ...
    rebuildRenderTargets() {
        ...
        if (!mainTarget) {
            ...
            const pickerDepthBuffer = createTexture('pickerDepthColor', width, height, PIXELFORMAT_RGBA16F);
            ...
            this.colorTarget = new RenderTarget({
                // Keep picking independent from the visible scene target. Sharing
                // cameraColor here lets two RenderTargets resize/recreate the same
                // WebGPU texture and can leave a queued frame sampling a dead view.
                colorBuffer: pickerDepthBuffer,
                depth: false,
                autoResolve: false
            });
            ...
            // Unified WebGPU splats have a single fragment color output. The
            // legacy renderer wrote a second selection attachment through MRT;
            // binding that target on WebGPU invalidates the whole pipeline.
            // Selection/lock/delete visualization now comes from the unified
            // work-buffer modifier, so the scene and splats share mainTarget.
            this.clearPass.init(this.mainTarget);
            ...
            // configure unified splat pass - single color target, no clears
            this.splatPass.init(this.mainTarget);
            ...
            const { colorTarget, workTarget } = this;
    ...
    fitClippingPlanes(cameraPosition: Vec3, forwardVec: Vec3) {
        ...
        // Keep a non-degenerate clip range while a model is loading or when a
        // source contains only one point. near === far clips the whole frame.
        const boundRadius = Math.max(1e-3, bound.halfExtents.length());
        ...
        this.far = Math.max(this.far, this.near + Math.max(1e-3, boundRadius * 1e-3));
    }
