    ...
    constructor(events: Events, tooltips: Tooltips, args = {}) {
        ...
        const rebuild = () => {
            ...
            const offsetFromFrame = (frame: number) => {
                return numFrames === 1 ? padding :
                    padding + Math.floor(frame / (numFrames - 1) * width);
            };

            frameFromOffset = (offset: number) => {
                return numFrames === 1 ? 0 :
                    Math.max(0, Math.min(numFrames - 1, Math.floor((offset - padding) / width * (numFrames - 1))));
            };
