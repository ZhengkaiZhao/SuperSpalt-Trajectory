...
class PointerController {
    ...
    constructor(camera: Camera, target: HTMLElement) {
        ...
        const look = (dx: number, dy: number) => {
            camera.look(dx, renderDeltaY(dy));
        };
        ...
        const pan = (x: number, y: number, dx: number, dy: number) => {
            ...
            const sourceY = renderY(y);
            const sourceDy = renderDeltaY(dy);
            c.screenToWorld(x, sourceY, distance, fromWorldPoint);
            c.screenToWorld(x - dx, sourceY - sourceDy, distance, toWorldPoint);
