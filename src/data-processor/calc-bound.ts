...
class CalcBound {
    ...
    async run(splat: Splat, selectionBound: BoundingBox, localBound: BoundingBox): Promise<void> {
        ...
        // WebGPU cannot submit a frame while an asynchronous staging buffer is
        // mapped for the bounds readback below. Pause on-demand rendering for
        // this short operation; keep the explicit offline render lock intact.
        const suspendInteractiveRendering = !scene.lockedRenderMode;
        if (suspendInteractiveRendering) {
