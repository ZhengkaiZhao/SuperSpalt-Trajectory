        ...
        if (isPlySequence(filenames)) {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (filename.endsWith('.ssproj')) {
                    ...
                    try {
                        let projectFile = files[i].contents;
                        if (!projectFile) {
                            if (!files[i].url) throw new Error('No data source was provided');
                            const response = await fetch(files[i].url);
                            if (!response.ok) throw new Error(`HTTP ${response.status}`);
                            const blob = await response.blob();
                            projectFile = new File([blob], files[i].filename, {
                                type: blob.type || 'application/x-supersplat'
                            });
                        }
                        await events.invoke('doc.load', projectFile, files[i].handle);
                    } catch (error) {
