...
const loadGSplatData = async (filename: string, fileSystem: ReadFileSystem, skipReorder?: boolean, pickLod?: PickLod): Promise<LoadResult | null> => {
    ...
    // Standard PLY rows can be hundreds of bytes wide. The library's default
    // 1M-row chunk makes a single 3DGS Range response roughly 250 MB, which
    // Chromium may abort before the streaming reader consumes it. Smaller
    // chunks preserve all source data while lowering network and CPU peaks.
    let sources: ChunkSource[];
    if (inputFormat === 'ply') {
        const source = await fileSystem.createSource(filename);
        try {
            sources = [await readPly(source, createChunkDataPool({ chunkSize: 64 * 1024 }))];
        } catch (error) {
            source.close();
            throw error;
        }
