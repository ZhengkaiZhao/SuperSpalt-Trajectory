class CubicSpline {
    ...
    static calcKnots(times: number[], points: number[], smoothness: number) {
        ...
        // Exact piecewise linear interpolation. Zero tangents produce a
        // smoothstep curve, not a line, and can make a dense camera path appear
        // to slow down or bow between its explicitly generated camera poses.
        if (smoothness === 0) {
            for (let i = 0; i < n; i++) {
                for (let j = 0; j < dim; j++) {
                    const index = i * dim + j;
                    const point = points[index];
                    const incoming = i > 0 ? point - points[index - dim] :
                        points[index + dim] - point;
                    const outgoing = i < n - 1 ? points[index + dim] - point :
                        point - points[index - dim];
                    knots[index * 3] = incoming;
                    knots[index * 3 + 1] = point;
                    knots[index * 3 + 2] = outgoing;
                }
            }
            return knots;
        }
