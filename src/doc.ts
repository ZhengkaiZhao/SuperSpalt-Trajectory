...
import { validateProjectDocument } from './project-document';
...
const registerDocEvents = (scene: Scene, events: Events) => {
    ...
    const loadDocument = async (file: File): Promise<boolean> => {
        ...
        let addedSplats = 0;
        let preferencesSuspended = false;

        try {
            // Validate and fully load all project assets before replacing the
            // current scene. A corrupt project must not destroy current work.
            const docSource = await zipFs.createSource('document.json');
            let docData: Uint8Array;
            try {
                if (docSource.size > 64 * 1024 * 1024) {
                    throw new Error('Invalid SuperSplat project: document.json is too large');
                }
                docData = await docSource.read().readAll();
            } finally {
                docSource.close();
            }
            const project = validateProjectDocument(JSON.parse(new TextDecoder().decode(docData)));

            for (let i = 0; i < project.splats.length; ++i) {
                const filename = `splat_${i}.ply`;
                const splat = await scene.assetLoader.load(filename, zipFs, false, true);
                if (!splat) throw new Error(`Unable to load ${filename}`);
                splat.docDeserialize(project.splats[i]);
                stagedSplats.push(splat);
            }

            // Document view settings use the same events as user changes. Only
            // suspend preference capture once preflight has succeeded.
            events.fire('preferences.suspend');
            preferencesSuspended = true;
            resetScene();
            ...
            events.invoke('docDeserialize.poseSets', project.poseSets, project.camera.fov);
            events.invoke('docDeserialize.view', project.view);
            scene.camera.docDeserialize(project.camera);
            ...
            // If commit had started, remove a partially loaded project. Most
            // failures occur during preflight, while the previous scene remains.
            if (addedSplats > 0) resetScene();
            for (let i = addedSplats; i < stagedSplats.length; i++) stagedSplats[i].destroy();
            ...
            if (preferencesSuspended) events.fire('preferences.resume');
            events.fire('stopSpinner');
            try {
                zipFs.close();
            } catch (error) {
                console.error('Failed to close project file', error);
            }
        }
    };
    ...
    events.function('doc.load', async (file: File, handle?: FileSystemFileHandle) => {
        ...
        if (!await loadDocument(file)) return false;
    ...
    events.function('doc.open', async () => {
        ...
        if (fileSelector) {
            fileSelector.show(async (file?: File) => {
                if (file && await loadDocument(file)) events.fire('doc.setName', file.name);
            });
        } else {
            try {
                ...
                if (fileHandles?.length === 1) {
                    ...
                    if (!await loadDocument(await fileHandle.getFile())) return false;
